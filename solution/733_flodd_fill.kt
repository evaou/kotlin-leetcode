/*
fun floodFill(image, sr, sc, newColor)
    seenImage = image size with 0 value
    directPixelList = emptyList

    getDirectPixelList(image, sr, sc, directPixelList, seenImage)

    update image with (sr, sc)+directPixelList with new Color

    return image

fun getDirectPixelList(image, sr, sc, directPixelList, seenImage)
    tmpPixelList = Filter 4-direct pixel with same color and under length limit and haven't been seen

    directPixelList += tmpPixelList

    go thru each pixel in tmpPixelList
        getDirectPixelList(image, sr, sc, directPixelList)
 */
